...
export default function SignupPage() {
  ...
  const labelStyle = {
    display: 'block' as const, fontSize: '11px', fontFamily: 'DM Mono, monospace',
    color: '#607276', letterSpacing: '0.8px', textTransform: 'uppercase' as const, marginBottom: '6px'
  }
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    if (data.user) {
      await supabase.from('profiles').upsert({
        id: data.user.id, username: form.username,
        streak_count: 0, badges: []
      })
    }
    setSuccess(true)
  ...
  if (success) return (
    <div style={{ minHeight: '100vh', background: '#eef3f4', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: '#607276' }}>Redirecting to your dashboard...</p>
      </div>
    </div>
  )
      ...
      <div style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,.62)', border: '1px solid rgba(38,69,72,.12)', borderRadius: '16px', padding: '28px' }}>
          <button onClick={handleGoogle} style={{
            width: '100%', padding: '11px', background: '#fff', border: 'none', borderRadius: '10px',
            cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
            gap: '10px', marginBottom: '20px', fontFamily: 'Syne, sans-serif',
            fontSize: '14px', fontWeight: 700, color: 'rgba(255,255,255,.62)'
          }}>
          ...
          <form onSubmit={handleSignup}>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              {loading ? (
                <><div style={{ width: '14px', height: '14px', border: '2px solid rgba(255,255,255,.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} /> Creating account...</>
              ) : 'Create Free Account →'}
            </button>
          </form>
          ...
          <p style={{ textAlign: 'center', marginTop: '20px', fontSize: '13px', color: '#607276' }}>
            ...
            <Link href="/login" style={{ color: '#5aa0a4', textDecoration: 'none', fontWeight: 600 }}>Sign in</Link>
          </p>
          <p style={{ textAlign: 'center', marginTop: '12px', fontSize: '11px', color: '#8a9a9d' }}>
